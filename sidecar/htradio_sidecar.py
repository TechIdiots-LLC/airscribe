...
Backends:
  sim    A fake radio that "hears" a burst of tone every few seconds. Lets the
         whole pipeline run with no Bluetooth hardware.
...
class SimBackend:
    ...
    def _run(self, mac, stop):
        ...
        while not stop.is_set():
            # Quiet, then a "transmission" of 2-4 s: a tone with syllable-like
            # amplitude wobble, so the energy detector has something to see.
            for talking, secs in ((False, random.uniform(2, 4)), (True, random.uniform(2, 4))):
                for i in range(int(secs * 1000 / CHUNK_MS)):
                    ...
                    emit({"event": "audio", "mac": mac, "rx": talking, "pcm": base64.b64encode(pcm).decode()})
